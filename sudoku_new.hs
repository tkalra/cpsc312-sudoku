...
--as a 2x2 matrix, then this function will take the row and column of the element to change and call the previous
...
verify_rows_str_impossible sudoku row =
	if row > 9 then False
	else has_duplicate (make_row_str sudoku row 1) || verify_rows_str_impossible sudoku (row + 1)
...
verify_columns_str_impossible sudoku column =
	if column > 9 then False
	else has_duplicate (make_column_str sudoku 1 column) || verify_columns_str_impossible sudoku (column + 1)
...
verify_squares_str_impossible sudoku row column =
	if row>7 then False
	else
		if column == 7 then has_duplicate (make_square_str sudoku row column row column) || verify_squares_str_impossible sudoku (row +3) 1
		else has_duplicate (make_square_str sudoku row column row column) || verify_squares_str_impossible sudoku row (column +3)

--This function uses the 3 previous ones to find whether any row, column or square is breaking the rules
is_trivially_impossible sudoku = verify_rows_str_impossible sudoku 1 || verify_columns_str_impossible sudoku 1 || verify_squares_str_impossible sudoku 1 1
...
run = do
	...
	if sudoku == [] then do return ()
		...
		if is_trivially_impossible sudoku then do
			...
			let (bool, solved) = solve_str sudoku
